        ...
        if len(all_points) > 0:
            ...
            for dis, st_size in zip(dis_list, st_sizes):
                if len(dis) > 0:
                    ...
                    gau = torch.sum(gau, dim=0).reshape(self.wid, self.wid)
                    if self.use_bg:
...
if __name__ == "__main__":
    post = Post_Prob(sigma=8.0, c_size=224, stride=16, background_ratio=0.15, use_background=True, device="cuda")
    points = [torch.abs(torch.randn(10, 2)).to("cuda")]
    st_size = [224]
    _, gau = post(points, st_size)
    print(gau.shape)
