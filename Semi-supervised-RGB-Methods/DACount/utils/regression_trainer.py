...
def train_collate(batch):
    ...
    return [rgb, t], points, targets, st_sizes, label
...
class RegTrainer(Trainer):
    def setup(self):
        ...
        self.dataloaders = {
            'train': DataLoader(self.datasets['train'], collate_fn=train_collate, batch_size=1,
                                 shuffle=True, num_workers=args.num_workers*self.device_count, pin_memory=False),
    ...
    def train_epoch(self, unlabel):
        ...
        for step, (inputs, points, targets, st_sizes, label) in enumerate(self.dataloaders['train']):
            assert inputs[0].size(0) == 1, 'sorry, the code now only supports one batch'
    ...
    def val_epoch(self):
        ...
        epoch_res = []
        # Iterate over data.
        for rgb, t, count, name, k in self.dataloaders['val']:
