...
class RegTrainer(Trainer):
    ...
    def train_epoch(self):
        ...
        # Iterate over data.
        dataloader = tqdm(self.dataloaders['train'], desc="Training", leave=False, dynamic_ncols=True)
        for step, (inputs, points, targets, gt_discrete, st_sizes, label) in enumerate(dataloader):
        # for step, (inputs, points, targets, gt_discrete, st_sizes, label) in enumerate(self.dataloaders['train']):
            ...
            with torch.set_grad_enabled(True):
                ...
                if label[0]:
                    loss = self.criterion(points, st_sizes, targets, outputs)
                else:
        ...
        logging.info('Epoch {} Train, Loss: {:.2f}, MSE: {:.2f} MAE: {:.2f}, Cost {:.1f} sec'
                     .format(self.epoch, epoch_loss.get_avg(), np.sqrt(epoch_mse.get_avg()), epoch_mae.get_avg(), time.time() - epoch_start))
    ...
    def val_epoch(self):
        ...
        mse = [0, 0, 0, 0]
        # Iterate over data.
        dataloader = tqdm(self.dataloaders['val'], desc="Validating", leave=False, dynamic_ncols=True)
